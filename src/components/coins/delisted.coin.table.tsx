...
const DelistedCoinsPage = () => {
    ...
    const getData = async () => {
        ...
        try {
            const res = await fetch(
                `http://localhost:5000/api/coins/new-delisted`,
                {
                    headers: {
            ...
            if (Array.isArray(coinsData)) {
                ...
                notification.error({
                    message: "No delisted coins found",
            ...
            console.error("Error fetching delisted coins:", error);
            notification.error({
                message: "Error fetching delisted coins",
    ...
    return (
        <div>
            <h2>Delisted Coins</h2>
            {isLoading ? (
                <Spin tip="Loading delisted coins..." size="large" style={{ display: "block", margin: "50px auto" }} />
            ) : (
                <Table
                    columns={columns}
...
export default DelistedCoinsPage;
